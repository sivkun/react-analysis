...
ReactDOMComponent.Mixin = {
  ...
  receiveComponent: function(nextElement, transaction, context) {
    ...
    this._currentElement = nextElement;//这里已经修改了_currentElement
  ...
  updateComponent: function(transaction, prevElement, nextElement, context) {
    ...
    this._updateDOMProperties(lastProps, nextProps, transaction, isCustomComponentTag); //dom组件更新主要更新其属性
  ...
  _updateDOMProperties: function(
    ...
    var propKey;
    var styleName;
    var styleUpdates; //style属性更新
    //首先，遍历老属性，如果属性不在新属性里面，要删除。
    for (propKey in lastProps) {
      //新的属性里有，或者propkey在原型链上、或者propkey对应的值为null。这样剩下的都是不在新属性集合里的，要删除。
      ...
      } else if (isCustomComponent(this._tag, lastProps)) { //不懂
        if (!RESERVED_PROPS.hasOwnProperty(propKey)) {
    ...
    for (propKey in nextProps) {
      ...
      if (propKey === STYLE) {
        ...
        if (lastProp) {
          ...
          for (styleName in lastProp) {
            if (lastProp.hasOwnProperty(styleName) &&
                (!nextProp || !nextProp.hasOwnProperty(styleName))) {//!nextProp多此一举不是吗
              styleUpdates = styleUpdates || {};
              styleUpdates[styleName] = '';
            }
          }
        ...
        if (nextProp) {
          ensureListeningTo(this, propKey, transaction); //注册事件
        }
  ...
  _updateDOMChildren: function(lastProps, nextProps, transaction, context) {
    //如果children为数字或者string，lastContent和nextContent将其保存，作为该dom的文本节点内容。
    var lastContent =
      CONTENT_TYPES[typeof lastProps.children] ? lastProps.children : null;
    var nextContent =
      CONTENT_TYPES[typeof nextProps.children] ? nextProps.children : null;
    //下面还不懂要干啥，先猜测，可能是通过innerHTML填加入的内容。
    var lastHtml =
