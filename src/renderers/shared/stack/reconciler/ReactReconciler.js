...
var ReactReconciler = {
  ...
  receiveComponent: function(
    ...
    //虽说在上一步进行了shouldUpdateReactComponent(prevRenderedElement, nextRenderedElement)，到这里说明type和key是相同的，这里做了更严格比较。
    if (nextElement === prevElement &&
        context === internalInstance._context
      ) {
      // Since elements are immutable after the owner is rendered,
      // we can do a cheap identity compare here to determine if this is a
      // superfluous（过分的） reconcile. It's possible for state to be mutable but such
    ...
    //这里又是在ReactReconcileTransaction的close阶段添加ref。
    if (refsChanged &&
        internalInstance._currentElement &&
        internalInstance._currentElement.ref != null) {
      transaction.getReactMountReady().enqueue(attachRefs, internalInstance);
